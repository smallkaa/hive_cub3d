...
void move_forward_backward(t_game *game, int forward)
{
    ...
    double speed;
     ...
     speed = forward ? MOVE_SPEED : -MOVE_SPEED;
    angle_rad = game->map->hero.angle * M_PI / 180.0;
    
    // FIXED: Correct direction calculation for cub3D coordinate system
    // North = 0°, East = 90°, South = 180°, West = 270°
    dx = cos(angle_rad) * speed;  // ← Changed from sin()
    dy = sin(angle_rad) * speed;  // ← Changed from -cos()
...
void strafe_left_right(t_game *game, int left)
{
    ...
    speed = left ? -MOVE_SPEED : MOVE_SPEED;  // ← Fixed direction
    angle_rad = (game->map->hero.angle + 90) * M_PI / 180.0;  // Perpendicular angle
...
void rotate_player(t_game *game, int left)
{
    ...
    rotation = left ? -ROTATION_SPEED : ROTATION_SPEED;
...
void handle_movement(t_game *game)
{
    ...
    if (mlx_is_key_down(game->mlx, MLX_KEY_A))
        strafe_left_right(game, 1);
    if (mlx_is_key_down(game->mlx, MLX_KEY_D))
        strafe_left_right(game, 0);
